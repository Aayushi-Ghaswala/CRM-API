        ...
        public async Task<int> AddNJMutualfundDetails(IFormFile file, bool UpdateIfExist)
        {
            ...
            using (var stream = File.Open(localFilePath, FileMode.Open, FileAccess.Read))
            {
                using (var reader = ExcelReaderFactory.CreateReader(stream))
                {
                    ...
                    if (mapRecordsForExistUser.Count > 0)
                    {
                        await _mutualfundRepositry.AddMutaulfundDetailsForExistUser(mapRecordsForExistUser);
                    }
                    //else if (mapRecordsForNotExistUser.Count > 0)
                    //{
                    //    await _mutualfundRepositry.AddMutaualfundDetailsToNotExistUserTable(mapRecordsForNotExistUser);
                    //}
                    await _mutualfundRepositry.AddMutaualfundDetailsToNotExistUserTable(mapRecordsForNotExistUser);
                }
                return 0;
