...
namespace CRM_api.Services.Helper.Reminder_Helper.LI_GI_Module
{
    ...
    {
        public static async void InsPremimumReminder(IConfiguration configuration, IInsuranceClientRepository insuranceClientRepository)
        {
            var insClients = await insuranceClientRepository.GetInsClientsForInsPremiumReminder();
            ...
            foreach (var insClient in insClients)
            {
                ...
                if (insClient.IsEmailReminder is true)
                {
                ...
                if (insClient.IsSmsReminder is true)
                {
                ...
                await insuranceClientRepository.UpdateInsuranceClientDetail(insClient);
            }
        }
    }
}
